...
CREATE TABLE IF NOT EXISTS ekko.voice_messages (
    ...
    avg_frequency DOUBLE NULL COMMENT '当前句子的包络波峰每秒个数（语速代理特征）',
...
CREATE TABLE IF NOT EXISTS ekko.user_channel_voice_profile (
    ...
    historical_avg_amplitude DOUBLE NOT NULL DEFAULT 0 COMMENT '该用户在该频道历史句子平均绝对振幅的平均值',
    historical_avg_frequency DOUBLE NOT NULL DEFAULT 0 COMMENT '该用户在该频道历史包络波峰每秒个数的平均值',
    historical_avg_char_rate DOUBLE NOT NULL DEFAULT 0 COMMENT '该用户在该频道历史转写文本每秒字数的平均值',
    char_rate_sample_count INT NOT NULL DEFAULT 0 COMMENT '纳入文本语速历史基线的样本数',
